define(['./primitives', './isNull'], function (primitives, isNull) {
    ...
    return {
        ...
        int64 : function (defaultValue, bytes, position) {
            return [
                primitives.int32(bytes, position+4) ^ defaultValue[0],
        ...
        pointer : {
            get : function (Type) {
                return function (defaultPosition, context, offset) {
                    ...
                    if (pointer.position < context._end && !isNull(pointer))
                        return Type._deref(context._arena, pointer, context._depth+1);
                    else
                        return context._pointerDefaults[defaultPosition];
                };
            },
            has : function (Type) {
