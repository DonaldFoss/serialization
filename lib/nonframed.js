define(['./reader/Arena', './builder/Allocator', './builder/copy/pointer'], function (
                  Arena,             Allocator,             copy) {
    ...
    var fromStruct = function (instance) {
        ...
        if (layout.meta !== 0)
            throw new TypeError('Message must have a structure as its root');
        ...
        if (arena._segments.length !== 1) {
            ...
            var size = 0;
            arena._segments.forEach(function (s) { size += s._position; });
