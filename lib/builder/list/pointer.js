define(['../../reader/list/meta', '../../type', '../copy/pointer', '../layout/index', './methods', './statics'], function (
                       listMeta,         type,      copy,              layout,           methods,     statics) {
    ...
    return function (Nonstruct) {
        ...
        Pointers.prototype.adopt = function (index, orphan) {
            if (!Nonstruct.equiv(orphan._TYPE))
                throw new TypeError();

            if (!orphan._isOrphan)
                throw new ValueError('Cannot adopt a non-orphan');

            if (!this._arena.isEquivTo(orphan._arena))
