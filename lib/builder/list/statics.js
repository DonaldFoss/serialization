define(['../../reader/layout/list', '../../reader/isNull', '../layout/list', '../copy/pointer', '../upgrade', './field/index'], function (
               reader,                            isNull,      layout,           copy,              upgrade,     field) {

    var deref = function (List) {
        List._deref = function (arena, pointer) {
            var ell = reader.unsafe(arena, pointer);

            if (ell.dataBytes < List._CT.dataBytes
             || ell.pointersBytes < List._CT.pointersBytes) {
