define(['../reader/isNull', '../reader/Arena', '../reader/layout/structure', '../wordAlign', './copy/pointer', './layout/structure', './upgrade'], function (
                   isNull,      Reader,            reader,                       wordAlign,     copy,                      builder,     upgrade) {
    ...
    Builder.prototype.initOrphan = function (Type, optionalLength) {
        if (optionalLength === undefined) {
            if (Type._CT.meta === 1)
                throw new Error("'initOrphan' for list types requires a length");
        } else {
            if (Type._CT.meta === 0)
                throw new Error("'initOrphan' for struct types requires no length");
        }
    ...
    Builder.prototype._preallocate = function (localSegment, bytes) {
        ...
        if (oldEnd + bytes <= localSegment.length) {
            ...
            if (bytes > 0) {
                localSegment.fill(
                    0,
                    localSegment._position - 8,
                    localSegment._position
                );
            }
            // @endif
    ...
    Builder.prototype._write = function (source, length, target) {
        ...
        target.segment.set(
            source.segment.subarray(source.position, source.position+length),
            target.position
        );
        // @endif
    ...
    Builder.prototype._allocateOrphan = function (bytes) {
        var s0 = this._segments[0];
        if (s0 && s0._position > 0) return this._allocate(bytes);
        else {
