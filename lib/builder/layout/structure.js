define(['../far'], function (far) {
    ...
    var intrasegment = function (pointer, blob, meta) {
        ...
        pointer.segment[pointer.position] = offset << 2; // A bits for free.
    ...
    var preallocated = function (pointer, blob, meta) {
        if (pointer.segment === blob.segment) {
            ...
            // @if TARGET_ENV='node'
            land.segment[land.position] = 0;
            land.segment[land.position+1] = 0;
            land.segment[land.position+2] = 0;
            land.segment[land.position+3] = 0;
            // @endif
