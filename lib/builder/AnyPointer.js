define(['../reader/AnyPointer', '../reader/isNull', './AnyPointerBlob'], function (
                       Reader,             isNull,               Blob) {
    ...
    Any._set = function (arena, pointer, value) {
        if (reader._TYPE === t) {
            ...
            if (value._arena.IS_READER) {
                ...
                builder.copy.pointer.deep(value, this._arena, pointer);
            } else {
                builder.copy.pointer.deep(value, this._arena, pointer);
            }
        }
    ...
    Any.prototype.cloneAsOrphan = function () {
        if (isNull(this._pointer))
            throw new ValueError('Attempted to clone a null AnyPointer');

// here's when the layout should fix
        return new Blob._deref(this._arena, this._pointer);
    };
});
