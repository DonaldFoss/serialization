define(['../reader/isNull', './copy/pointer', './primitives'], function (
                   isNull,     copy,             primitives) {
        ...
        pointer : {
            adopt : function (Type) {
                return function (context, offset, value) {
                    if (!Type._TYPE.equiv(value._TYPE))
                        throw new TypeError();

                    if (!value._isOrphan)
                        throw new ValueError('Cannot adopt a non-orphan');

                    if (!this._arena.isEquivTo(value._arena))
            ...
            disown : function (Type) {
                return function (context, offset) {
                    ...
                    if (isNull(pointer)) {
                        /*
                         * For lists, there's no length to use to initialize on
                         * null, so throw (and extrapolate the behavior to
                         * structs).
                         */
                        throw new Error('Attempted to disown a null pointer');
                    } else {
                        var instance = Type._deref(
                            context._arena,
                            pointer
                        );
            ...
            disownReader : function (ReaderType) {
                return function (context, offset) {
                    ...
                    if (isNull(pointer)) {
                         ...
                         * structs).
                         */
                        throw new Error('Attempted to disown-as-reader a null pointer');
                    } else {
                        var instance = Type._deref(
                            context._arena,
                            pointer,
        ...
        struct : {
            get : function (Structure) {
                return function (defaultPosition, context, offset) {
                    ...
                    if (isNull(pointer)) {
                        var d = context._pointerDefaults[defaultPosition];
                        copy.setStructPointer( // Only difference from list.
                            d._arena,
                            d._layout(),
                            context._arena,
                            pointer
