define(['../reader/layout/index', '../reader/isNull', '../reader/far', '../reader/list/meta', './primitives',    './far', './layout/index', './shiftOffset'], function (
            reader,                          isNull,       farReader,                  meta,         encode, farBuilder,          builder,     shiftOffset) {
    ...
    var far = function (arena, pointer) {
         ...
         */
        if (pointer.segment[pointer.position] & 0x04) return;
        ...
        if (pointer.segment === blob.segment) {
            ...
            encode.int32(
                (offset>>1) | (tag.segment[tag.position] & 0x03),
                pointer.segment,
                pointer.position
            );
            ...
            arena._zero(tag, 8);
        } // Else leave the far pointer as-is
    };
    ...
    var structure = function (arena, pointer, ct) {
        ...
        if (layout.segment === blob.segment) {
            ...
            intrasegmentMovePointers(
                iTarget,
                rtPointers >>> 3,
                layout.pointersSection - iTarget.position
            );

            // @if TARGET_ENV='node'
            arena._zero(iTarget, ct.pointersBytes - rtPointers);
            // @endif
