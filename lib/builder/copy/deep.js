define(['../../reader/layout/any', '../../reader/list/meta', '../../reader/isNull', '../layout/structure', '../layout/list'], function(
                             any,                     meta,                isNull,             structure,      layout) {
    ...
    var setStructurePointer = function (arena, layout, targetArena, target) {
        var blob = targetArena._preallocate(
            target.segment,
    ...
    var setListPointer = function (arena, ell, targetArena, target) {
        ...
        if (m.layout === 0x01) {
            blob = targetArena._preallocate(
                target.segment,
                (ell.length>>>3) + (ell.length & 0x07 ? 1 : 0)
            );
        } else if (m.layout === 0x07) {
            // Add an extra 8 bytes for the tag word.
            blob = targetArena._preallocate(
                target.segment,
                8 + ell.length * (ell.dataBytes + ell.pointersBytes)
            );
        } else {
            blob = targetArena._preallocate(
                target.segment,
                ell.length * (ell.dataBytes + ell.pointersBytes)
            );
        }
