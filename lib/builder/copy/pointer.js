define(['../../reader/layout/any', '../../reader/list/meta', '../../reader/isNull', '../layout/structure', '../layout/list', './blob'], function(
                             any,                     meta,                isNull,             structure,             list,     blob) {
    ...
    var setStructPointer = function (arena, layout, targetArena, target) {
        var b = targetArena._preallocate(
            target.segment,
    ...
    var setListPointer = function (arena, layout, targetArena, target) {
        ...
        var b;
        if (m.layout === 0x07) {
            b = targetArena._preallocate(
                target.segment,
                8 + layout.length * (layout.dataBytes + layout.pointersBytes)
            );
        } else if (m.layout === 0x01) {
            b = targetArena._preallocate(
                target.segment,
                (layout.length>>>3) + (layout.length & 0x07 ? 1 : 0)
            );
        } else {
            b = targetArena._preallocate(
                target.segment,
                layout.length * (layout.dataBytes + layout.pointersBytes)
            );
        }
