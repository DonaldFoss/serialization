define(['./reader/Arena', './reader/primitives', './builder/primitives'], function (
                  Arena,     reader,                builder) {
    ...
    var fromStruct = function (instance) {
        ...
        var bytes = new Uint8Array(((count & 0xfffe) + 1) << 3);
        // @endif

        // @if TARGET_ENV='node'
        var bytes = new Buffer(((count & 0xfffe) + 1) << 3);
        // @endif

        builder.uint32(count, bytes, 0);
        for (var i=0; i<arena._segments.length; ++i) {
            builder.uint32(
                arena._segments[i]._position >>> 3,
                bytes,
                (1+i) << 2
            );
        }
